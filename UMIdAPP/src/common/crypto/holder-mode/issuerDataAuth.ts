...
export function verifySignature(cert,mso){
  ...
  let mso_data = GeneralMethods.url_safe_base64_to_base64(mso);
...
export function get_DS_certificate(signature){
  let signature_decoded = GeneralMethods.url_safe_base64_to_base64(signature);
  console.log('signature_decoded: ', signature_decoded);
  let readable_signature = CBOR.decode(Buffer.from(signature_decoded,'base64').buffer)
  ...
  let cert = GeneralMethods.decodeCert(pemText);
  return cert;
}
