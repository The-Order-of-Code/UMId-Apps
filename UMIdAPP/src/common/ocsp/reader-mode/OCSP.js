...
export function createOCSPResquest(certificate) {
  ...
  const authorityKeyIdentifier = _.find(
    certificate.extensions,
    (extension) => extension.extnID === '2.5.29.35'
  );
  const issuerKeyHash =
    authorityKeyIdentifier.parsedValue.keyIdentifier.valueBlock.valueHex;
  ...
  ocspReqSimpl.tbsRequest.requestList = [
    new pkijs.Request({
  ...
  ocspReqSimpl.tbsRequest.requestExtensions = [
    new pkijs.Extension({
      ...
      extnValue: new asn1js.OctetString({ valueHex: ocspNonceValue }).toBER(
        false
      ),
    }),
  ];
  ...
  const contentPem = Buffer.from(new Uint8Array(ocspReqBuffer)).toString(
    'base64'
  );
...
export async function sendOCSPRequest(request) {
  ...
  //console.log('https://vhaslab05.inesctec.pt/ocsp/'+ URIrequest);
  ...
  if (sizeRequest <= 255) {
    ...
    const der = Buffer.from(request, 'base64');
  ...
  const link = `https://vhaslab05.inesctec.pt/ocsp/${
    sizeRequest <= 255 ? URIrequest : ''
  }`;
  ...
  const response = http.sendRequest(link, reqOptions).then(
    (result) => {
      ...
      console.log('bugger der:', derBuffer, result.data);
      const resultResponse =
        prefix + derBuffer.match(/.{0,64}/g).join('\n') + postfix;
...
export function verifyOCSPResponse(response) {
  ...
  var ocspResponseBuffer = response.replace(
    /(-----(BEGIN|END) OCSP RESPONSE-----|[\n\r])/g,
    ''
  );
  let der = new Buffer.from(ocspResponseBuffer, 'base64');
  ...
  if ('responseBytes' in ocspRespSimpl) {
    const asn1Basic = asn1js.fromBER(
      ocspRespSimpl.responseBytes.response.valueBlock.valueHex
    );
  ...
  let signatureAlgorithm =
    algomap[ocspBasicResp.signatureAlgorithm.algorithmId];
  ...
  if (
    ocspBasicResp.tbsResponseData.responderID instanceof
    ...
    for (
      let i = 0;
      i < ocspBasicResp.tbsResponseData.responderID.typesAndValues.length;
      i++
    ) {
      let typeval =
        typemap[
          ocspBasicResp.tbsResponseData.responderID.typesAndValues[i].type
        ];
      ...
      const subjval =
        ocspBasicResp.tbsResponseData.responderID.typesAndValues[i].value
          .valueBlock.value;
    ...
    if (
      ocspBasicResp.tbsResponseData.responderID instanceof asn1js.OctetString
    ) {
      console.log(
        bufferToHexCodes(
          ocspBasicResp.tbsResponseData.responderID.valueBlock.valueHex,
          0,
          ocspBasicResp.tbsResponseData.responderID.valueBlock.valueHex
            .byteLength
        )
  ...
  for (let i = 0; i < ocspBasicResp.tbsResponseData.responses.length; i++) {
    const serialNumber = bufferToHexCodes(
      ocspBasicResp.tbsResponseData.responses[i].certID.serialNumber.valueBlock
        .valueHex
    );
    var resultOCSP = '';

    switch (
      ocspBasicResp.tbsResponseData.responses[i].certStatus.idBlock.tagNumber
    ) {
      case 0:
        resultOCSP = 'good';
        break;
