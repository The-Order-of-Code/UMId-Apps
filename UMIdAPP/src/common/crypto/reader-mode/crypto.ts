...
import * as General from '../../general/reader-mode/general';
...
export async function decrypt_msg(
  ...
  let msg;
  try{
    msg = await ComunicationCrypto.decrypt_msg(
      session_key,
      data,
      iv,
      additional_data
    ...
    const holder_response = CBOR.decode(msg); // para a comunicação offine
